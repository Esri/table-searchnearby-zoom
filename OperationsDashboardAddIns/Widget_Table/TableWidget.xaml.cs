...
namespace OperationsDashboardAddIns
{
  ...
  public partial class TableWidget : UserControl, IWidget, IDataSourceConsumer
  {
     ...
     private int initiatedMapWidgetCount = 0;
    ...
    {
       //region listen to text size mode changed event and responsd to the same in your widget
      ...
      //we will check that all map widgets have been initiated before adding the feature actions to the context menu
      List<MapWidget> mapWidgets = OperationsDashboard.Instance.Widgets.Where(widget => widget is MapWidget).Cast<MapWidget>().ToList();
      foreach (MapWidget mw in mapWidgets)
      {
         mw.Initialized += (sender, e) =>
            {
               //Increment the count of initiated map widgets, when all are initiated, 
               //data sources will be ready to be consumed by the search nearby tool
               //and so we will populate the feature action context menu
               initiatedMapWidgetCount++;

               if (initiatedMapWidgetCount == OperationsDashboard.Instance.Widgets.Where(widget => widget is MapWidget).Cast<MapWidget>().Count())
               {
    ...
    /// <summary>
    /// Persist user's selected feature actions and their properties
    /// </summary>
    private void InitializePersistedFeatureActions()
    {
       ...
       foreach (var featureAction in FeatureActions)
       {
          ...
          else if (featureAction is SearchNearbyFeatureAction)
          {
             ...
             if (((SearchNearbyFeatureAction)featureAction).TargetDataSource == null)
                TargetDataSourceId = OperationsDashboard.Instance.DataSources.First(ds => ds != null).Id;
             else
                TargetDataSourceId = ((SearchNearbyFeatureAction)featureAction).TargetDataSource.Id;
          }
          else
    ...
    /// <summary>
    /// Initialize the available feature actions using thre previously persisted feature actions
    /// </summary>
       ...
       foreach (var persistedFeatureAction in PersistedFeatureActions)
       {
          switch (persistedFeatureAction)
          {
             ...
             case FeatureActionType.SearchNearby:
                {
                   ESRI.ArcGIS.OperationsDashboard.DataSource TargetDataSource = OperationsDashboard.Instance.DataSources.FirstOrDefault(ds => ds.Id == TargetDataSourceId);
                   if (TargetDataSource != null)
                      featureActions.Add(new SearchNearbyFeatureAction(TargetDataSource, BufferDistance, BufferUnit));
                   else
                      featureActions.Add(new SearchNearbyFeatureAction(null, BufferDistance, BufferUnit));
                   break;
                }
